...
import { useUpsertPaymentMethod } from "../../hooks/payment/useUpsertPaymentMethod";
...
const PaymentMethodUpsertModal = ({ open, onClose, item, onSuccess }: Props) => {
  const isEdit = !!item;
  const [name, setName] = useState("");
  const { mutate: upsertMethod, isPending } = useUpsertPaymentMethod();
  ...
  const handleSubmit = () => {
    ...
    upsertMethod(payload, {
      onSuccess: (res: any) => {
        toast.success(res?.statusMessage || `Payment method ${isEdit ? "updated" : "added"} successfully`);
        onSuccess();
        onClose();
      },
      onError: (err: any) => {
          ...
          <div className="space-y-4">
            <div className="space-y-1.5 transition-all">
              ...
              <input
                ...
                placeholder="Enter method name (e.g. Cash, UPI)"
                value={name}
...
export default PaymentMethodUpsertModal;
