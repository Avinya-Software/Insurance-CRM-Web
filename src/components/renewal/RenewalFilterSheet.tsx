...
const RenewalFilterSheet = ({ open, filters, onApply, onClear, onClose }: Props) => {
  const [local, setLocal] = useState(filters);
  const { data: customers, isLoading: loadingCustomers } = useCustomerDropdown();
  const { data: statuses, isLoading: loadingStatuses } = useRenewalStatuses();

  const loading = loadingCustomers || loadingStatuses;
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-96 bg-white h-full shadow-xl flex flex-col">
        ...
        <div className="px-6 py-4 border-b flex justify-between items-center">
          <h2 className="font-semibold text-lg">Filter Renewals</h2>
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1.5 relative">
               <label className="text-sm font-medium">Customer</label>
                <SearchableComboBox
                    ...
                    onSelect={(item: any) =>
                    setLocal({ ...local, customerId: item?.value || "" })
                    }
                />
              </div>
              ...
              <Select
                ...
                value={local.renewalStatusId}
                options={statuses}
                valueKey="renewalStatusId"
                labelKey="statusName"
                onChange={(v: any) =>
                  setLocal({ ...local, renewalStatusId: v ? Number(v) : "" })
                }
              />
            </div>
          )}
...
export default RenewalFilterSheet;
  ...
  <div className="space-y-1.5 relative">
    ...
    <div className="relative">
      <select
        ...
        className="w-full px-4 py-2 border rounded appearance-none focus:ring-2 focus:ring-blue-50 focus:border-blue-400 outline-none"
      >
